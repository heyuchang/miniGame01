...
export class InfrastManager {
    ...
    private static _instance: InfrastManager;
    ...
    public static get instance () {
        ...
        this._instance = new InfrastManager();
    ...
    public showBanner(callback?: Function){
        ...
        // 微信小游戏
        if (sys.platform === sys.Platform.WECHAT_GAME && InfrastManager.adsBannerId !== ""){
            let { screenWidth, screenHeight } = wx.getSystemInfoSync()
            callback && callback()
            if (InfrastManager.bannerAd !== null){
                InfrastManager.bannerAd.destroy()
            }
            InfrastManager.bannerAd = wx.createBannerAd({
                adUnitId: InfrastManager.adsBannerId,
                style: {
                    left: 0,
                    top: screenHeight - 90,
                    width: screenWidth
            ...
            InfrastManager.bannerAd.onLoad(()=>{
                InfrastManager.bannerAd.show()
            })
            return
        }
        // 字节小游戏
        if (sys.platform === sys.Platform.BYTEDANCE_MINI_GAME && InfrastManager.adsBannerId !== ""){
            let { screenWidth, screenHeight } = tt.getSystemInfoSync()
            callback && callback()
            if (InfrastManager.bannerAd !== null){
                InfrastManager.bannerAd.destroy()
            }
            InfrastManager.bannerAd = tt.createBannerAd({
                adUnitId: InfrastManager.adsBannerId,
                style: {
                    left: 0,
                    top: screenHeight - 90,
    ...
    public hideBanner(callback?: Function){
        ...
        if (sys.platform === sys.Platform.WECHAT_GAME && InfrastManager.adsBannerId !== ""){
            callback && callback()
            if (InfrastManager.bannerAd !== null){
                InfrastManager.bannerAd.hide()
                InfrastManager.bannerAd.destroy()
                InfrastManager.bannerAd = null
            }
            return
        }
        // 字节小游戏
    ...
    public showInsAd(callback?: Function){
        ...
        // 微信小游戏
        if (sys.platform === sys.Platform.WECHAT_GAME && InfrastManager.interstitialAd !== null){
            callback && callback()
            InfrastManager.interstitialAd.show()
            return
        }
        // 字节小游戏
        if (sys.platform === sys.Platform.BYTEDANCE_MINI_GAME && InfrastManager.interstitialAd !== null){
            callback && callback()
            InfrastManager.interstitialAd.show()
            return
        }
    ...
    showVideoAd(callback?: Function,fcallback?: Function, videoKey:string = "default") {
        ...
        callback&&callback()
        return
        InfrastManager.adsVideoKey = videoKey
        // 安卓端
        if(sys.platform === "ANDROID"){
            game.pause()
            this.callBack = callback
        ...
        // 微信小游戏
        if (sys.platform === sys.Platform.WECHAT_GAME && InfrastManager.rewardedVideoAd !== null){
            game.pause()
            // console.log("调用广告")
            this.callBack = callback
            this.fcallBack = fcallback
            InfrastManager.rewardedVideoAd.show()
            .catch(err => {
                fcallback && fcallback()
                game.resume()
                InfrastManager.rewardedVideoAd.load()
            })
            return
        }
        // 字节小游戏
        if (sys.platform === sys.Platform.BYTEDANCE_MINI_GAME && InfrastManager.rewardedVideoAd !== null){
            this.callBack = callback
            this.fcallBack = fcallback
            game.pause()
            InfrastManager.rewardedVideoAd.show()
